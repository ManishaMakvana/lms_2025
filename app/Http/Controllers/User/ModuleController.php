...
use App\Mail\ModuleUnlockedMail;
...
class ModuleController extends Controller
{
    ...
    /**
     * Get the next available activation code for a module
     */
    public function getNextCode(TinkeringModule $module)
    {
        try {
            \Log::info('getNextCode method called', [
                'module_id' => $module->id,
            ...
            $nextCode = $module->getNextAvailableCode();
            
            \Log::info('Getting next code for module', [
                'module_id' => $module->id,
                'module_name' => $module->module_name,
                'next_code' => $nextCode
            ]);
            ...
            return response()->json([
                'success' => true,
                'code' => $nextCode,
                'message' => $nextCode ? 'Code available' : 'No available codes for this module'
            ]);
        } catch (\Exception $e) {
            \Log::error('Error getting next code', [
                'module_id' => $module->id,
                'error' => $e->getMessage()
            ]);
    ...
    public function redeemCode(RedeemCodeRequest $request, TinkeringModule $module)
    {
        try {
            ...
            $existingAccess = ModuleUser::where('user_id', $user->id)
                ->where('module_id', $module->id)
                ->first();

            if ($existingAccess) {
                return back()->withErrors(['code' => 'You already have access to this module.']);
            }

            // Check if this specific code has already been used by this user
            $codeAlreadyUsed = ModuleUser::where('user_id', $user->id)
                ->where('activation_code_id', $kitCode->id)
                ->first();

            if ($codeAlreadyUsed) {
                return back()->withErrors(['code' => 'This code has already been used by you. Please try a different code.']);
            }
            ...
            try {
                Mail::to($user->email)->send(new ModuleUnlockedMail($user, $module, $kitCode));
                \Log::info("Module unlock email sent to {$user->email} for module {$module->module_name}");
            } catch (\Exception $emailException) {
                \Log::error("Failed to send module unlock email: " . $emailException->getMessage());
                // Don't fail the entire process if email fails
            }
            
            return redirect()->route('modules.show', $module)
                ->with('success', 'Module unlocked successfully! Check your email for confirmation.');
